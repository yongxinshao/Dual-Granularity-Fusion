...
class DETR3DHead(DETRHead):
    ...
    Args:
        ...
        code_weights (List[double]) : loss weights of
            (cx,cy,l,w,cz,h,sin(蠁),cos(蠁),v_x,v_y)
    ...
    def forward(self, mlvl_feats: List[Tensor], img_metas: List[Dict],
        ...
        Returns:
            ...
            all_bbox_preds (Tensor): Sigmoid outputs from the regression
                ...
                (cx, cy, l, w, cz, h, sin(蠁), cos(蠁), vx, vy).
    ...
    def get_targets(
        ...
        Args:
            ...
            batch_bbox_preds (list[Tensor]): Sigmoid outputs from a single
                ...
                (cx,cy,l,w,cz,h,sin(蠁),cos(蠁),v_x,v_y) and
                shape [num_query, 10]
            batch_gt_instances_3d (list[:obj:`InstanceData`]): Batch of
                gt_instance.  It usually includes ``bboxes_3d``銆乣`labels_3d``.
        Returns:
    ...
    def loss_by_feat(
        ...
        Args:
            batch_gt_instances_3d (list[:obj:`InstanceData`]): Batch of
                gt_instance_3d.  It usually includes ``bboxes_3d``銆乣
                `labels_3d``銆乣`depths``銆乣`centers_2d`` and attributes.
                gt_instance.  It usually includes ``bboxes``銆乣`labels``.
