...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    # respectively
    def visualize_mask_opencv(self,mask):
        """
        浣跨敤 OpenCV 灏嗗崟閫氶亾鎺╃爜杞崲涓?BGR 褰╄壊鍥惧儚

        鍙傛暟:
            mask: 鍗曢€氶亾鎺╃爜鏁扮粍 (H, W)
        ...
        # 瀹氫箟棰滆壊 (BGR 鏍煎紡锛屽洜涓?OpenCV 浣跨敤 BGR)
        # 0: #804080 -> RGB(128, 64, 128) -> BGR(128, 64, 128)
        bgr_mask[mask == 0] = [128, 64, 128]

        # 1: #00008e -> RGB(0, 0, 142) -> BGR(142, 0, 0)
        bgr_mask[mask == 1] = [142, 0, 0]

        # 2: #DC143C -> RGB(220, 20, 60) -> BGR(60, 20, 220)
        bgr_mask[mask == 2] = [60, 20, 220]

        # 3: #0000e6 -> RGB(0, 0, 230) -> BGR(230, 0, 0)
        bgr_mask[mask == 3] = [230, 0, 0]
    ...
    def add_datasample(self,
        ...
        if gt_img_data is not None and pred_img_data is not None:
            ...
            drawn_img = None
        if drawn_img_3d is not None:
            mask = data_sample.seg_img[0].cpu().numpy().argmax(0)
            seg_mask = self.visualize_mask_opencv(np.uint8(mask))
            concatenated_img = np.vstack([drawn_img_3d, seg_mask])
        else:
