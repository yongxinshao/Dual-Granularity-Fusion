...
class StackedSAModuleMSG(BaseModule):
    ...
    def forward(self,
        ...
        for k in range(len(self.groupers)):
            ...
            grouped_features = grouped_features.permute(1, 0, 2).unsqueeze(dim=0)  # B*(M1 + M2, Cin, nsample)

            new_features = self.mlps[k](grouped_features)  # B*(M1 + M2, Cin, nsample)
            # (M1 + M2 ..., Cout, nsample)
            if self.pool_mod == 'max':
                new_features = new_features.max(-1).values  # B*(N1+N2)*nsample
