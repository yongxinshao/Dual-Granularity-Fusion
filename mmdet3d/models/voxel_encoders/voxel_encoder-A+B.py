    ...
    def forward(self,
        ...
        for i, vfe in enumerate(self.vfe_layers):
            ...
            if i == len(self.vfe_layers) - 1:
                ...
                fusion_point_feats = point_feats + image_featuress  # 铻嶅悎鍚庣殑鐗瑰緛
                sample_feature, batch_features, key_points = self.score_cross_attention(batch_size, coors,
                                                                                        fusion_point_feats,
                                                                                        score, points)
                seg_features = self.seg_encoder(fusion_point_feats, coors, batch_size, key_points, sample_feature,
                                                points)
                fusion_sample_feature = seg_features['fusion_feature']
                seg_result = self.semantic_head(seg_features['seg_feature'])
                score = seg_result['seg_preds'].sigmoid().max(dim=-1, keepdim=True).values
                key_xyz_batch = [len(p) for p in key_points]
                score_seg = []
                current_index = 0
                batch_aug_feature = []
                for k, index in enumerate(key_xyz_batch):
                    batch_seg_score = score[current_index:current_index + index]
                    # batch_aug_feature.append(fusion_sample_feature[k])
                    batch_aug_feature.append(fusion_sample_feature[k] * batch_seg_score)
                    score_seg.append(batch_seg_score)
                    current_index = current_index + index
                attention_list = self.CrossAttention_layers[i](batch_features, batch_aug_feature)
                point_feats = torch.cat(attention_list, 0) + fusion_point_feats
                # point_feats = self.fnn_layer(point_feats)
