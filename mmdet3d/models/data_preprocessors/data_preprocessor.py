...
class Det3DDataPreprocessor(DetDataPreprocessor):
    ...
    def simple_process(self, data: dict, training: bool = False) -> dict:
        ...
        data = self.collate_data(data)  # guiyihua 褰掍竴鍖?        inputs, data_samples = data['inputs'], data['data_samples']
        batch_inputs = dict()
        ...
        if 'paintimgs' in inputs:
            batch_inputs['paintimgs'] = inputs['paintimgs']
    ...
    def collate_data(self, data: dict) -> dict:
        ...
        Returns:
            dict: Data in the same format as the model input.
        """
        data = self.cast_data(data)  # type: ignore 鏀?
        if 'img' in data['inputs']:
            ...
            if is_seq_of(_batch_imgs, torch.Tensor):
                ...
                if img_dim == 3:
                    ...
                    # batch_imgs = stack_batch(batch_imgs, self.pad_size_divisor,self.pad_value)
                elif img_dim == 4:
