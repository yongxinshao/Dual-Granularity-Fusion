    ...
    def loss_by_feat(
        ...
        Args:
            ...
            batch_gt_instances_3d (list[:obj:`InstanceData`]): Batch of
                gt_instance_3d.  It usually includes ``bboxes``銆乣`labels``
                銆乣`bboxes_3d``銆乣`labels_3d``銆乣`depths``銆乣`centers_2d`` and
                attributes.
            batch_img_metas (list[dict]): Meta information of each image, e.g.,
                image size, scaling factor, etc.
    ...
    def get_targets(
        ...
        Args:
            points (list[Tensor]): Points of each fpn level, each has shape
                (num_points, 2).
            batch_gt_instances_3d (list[:obj:`InstanceData`]): Batch of
                gt_instance_3d.  It usually includes ``bboxes_3d``銆?                ``labels_3d``銆乣`depths``銆乣`centers_2d`` and attributes.
