...
class VoxelSetAbstraction(BaseModule):
    ...
    def forward(self, batch_inputs_dict: dict, feats_dict: dict,
        ...
        # keypoints = self.sample_key_points(batch_inputs_dict['key_points'],voxels_coors)
        # b*1024*3
        point_features_list = []
        ...
        key_xyz_batch_cnt = key_xyz.new_zeros(batch_size).int().fill_(
            num_keypoints)
        # key_xyz_batch_cnt[2048,2048]
        if self.rawpoints_sa_layer is not None:
            batch_points = torch.cat(points, dim=0)  # (N1+N2)*4
            batch_cnt = [len(p) for p in points]  # [N1,N2]
            xyz = batch_points[:, :3].contiguous()  # (N1+N2)*3
            features = None
            if batch_points.size(1) > 0:
                features = batch_points[:, 3:].contiguous()
            xyz_batch_cnt = xyz.new_tensor(batch_cnt, dtype=torch.int32) # tensor [N1,N2]
