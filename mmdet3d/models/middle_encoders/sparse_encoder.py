...
@MODELS.register_module()
class Voxel_Points_SparseEncoder(nn.Module):
    def __init__(
            self,
            in_channels: int,
            sparse_shape: List[int],
            ...
            order: Optional[Tuple[str]] = ('conv', 'norm', 'act'),
            Cro_attention_list: Optional[list] = None,
            bev_fusion: Optional[dict] = None,
            norm_cfg: Optional[dict] = dict(
                type='BN1d', eps=1e-3, momentum=0.01),
            base_channels: Optional[int] = 16,
            output_channels: Optional[int] = 128,
        ...
        self.img_transform = nn.Sequential(
            nn.Linear(32 + 64 + 128 * 2, 128),
            nn.BatchNorm1d(128, eps=1e-3, momentum=0.01),
        )
        if Cro_attention_list is not None:
            self.CrossAttention_layer_list = Cro_attention_list
            self.cro_sa_layers = nn.ModuleList()
            for Cro_attention_config in Cro_attention_list:
                cro_sa_layer = MODELS.build(Cro_attention_config)
                self.cro_sa_layers.append(cro_sa_layer)
        else:
            self.CrossAttention_layer_list = None
        self.bev_fusion = None
        if bev_fusion is not None:
            self.bev_fusion = MODELS.build(bev_fusion)

    @amp.autocast(enabled=False)
    def forward(self, voxel_features: Tensor, coors: Tensor,
                batch_size: int, key_points: List = None, score: List = None) -> Union[Tensor, Tuple[Tensor, list]]:
        """
        ...
        """
        coors = coors.int()
        new_xyz = torch.cat(key_points, dim=0)[:, :3]
        new_xyz_batch_cnt = new_xyz.new_zeros(batch_size).int()
        for bs_idx in range(batch_size):
            new_xyz_batch_cnt[bs_idx] = len(key_points[bs_idx])
        input_sp_tensor = SparseConvTensor(voxel_features, coors, self.sparse_shape, batch_size)
        x = self.conv_input(input_sp_tensor)
        encode_features = []
        pooled_features = []
        for i, voxel_sa_layer in enumerate(self.voxel_sa_layers):
            ...
            if new_xyz_batch_cnt.sum() > 0:
                cur_coords = x.indices
                xyz = self.get_voxel_centers(
                    coors=cur_coords,
                    scale_factor=self.voxel_sa_configs_list[i].scale_factor
                ).contiguous()
                xyz_batch_cnt = xyz.new_zeros(batch_size).int()
                for bs_idx in range(batch_size):
                    xyz_batch_cnt[bs_idx] = (cur_coords[:, 0] == bs_idx).sum()

                pooled_points, pooled_feature = voxel_sa_layer(
                    xyz=xyz.contiguous(),
                    xyz_batch_cnt=xyz_batch_cnt,
                    new_xyz=new_xyz.contiguous(),
                    new_xyz_batch_cnt=new_xyz_batch_cnt,
                    features=x.features.contiguous(),
                )
                if self.CrossAttention_layer_list is not None:
                    x = x.replace_feature(
                        features_layers_fusion(self.cro_sa_layers[i], batch_size, pooled_feature, new_xyz_batch_cnt, x))
                pooled_features.append(pooled_feature)
            encode_features.append(x)
        out = self.conv_out(encode_features[-1])
        if self.bev_fusion is not None and new_xyz_batch_cnt.sum() > 0:
            aug_features = self.img_transform(torch.cat(pooled_features, 1))
            out = out.replace_feature(
                features_layers_fusion(self.bev_fusion, batch_size, aug_features, new_xyz_batch_cnt, out))
        spatial_features = out.dense()
        N, C, D, H, W = spatial_features.shape
        spatial_features = spatial_features.view(N, C * D, H, W)
        if self.return_middle_feats:
...
def features_layers_fusion(cro_sa_layers, batch_size, aug_features, new_xyz_batch_cnt, out):
    attention_features = []
    for bs_idx in range(batch_size):
        if bs_idx == 0:
            sample_features = aug_features[:new_xyz_batch_cnt[0]].unsqueeze(0)
        else:
            in1 = new_xyz_batch_cnt[bs_idx - 1]
            in2 = new_xyz_batch_cnt[bs_idx] + in1
            sample_features = aug_features[in1:in2].unsqueeze(0)
        indx = out.indices[:, 0] == bs_idx
        batch_features = out.features.contiguous()[indx].unsqueeze(0)
        attention_features.append(cro_sa_layers(batch_features, sample_features).squeeze(0))
    return torch.cat(attention_features, 0)
