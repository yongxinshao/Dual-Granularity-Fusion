...
randomness = dict(seed=306874747)
...
train_pipeline = [
    ...
    # 鑰冭檻鐐逛簯filp transform3d208
    # dict(
    #     type='Img_Resize', scale=(1280, 400), keep_ratio=True),
    dict(
...
test_pipeline = [
    ...
    dict(type='Pack3DDetInputs', keys=['points', 'img', 'paintimgs'])
]
...
model = dict(
    ...
    pts_voxel_encoder=dict(
        type='PaintingDynamicVFE',
        in_channels=4,  # use_rgb 3 use_score:4 use_key_points 1
        feat_channels=[64, 64],
        with_distance=False,
        voxel_size=voxel_size,
        sample_points=[1024, 1024],
        ...
        fusion_layer=dict(
            type='PaintingFusion',
            img_channels=256,  # 涓轰粈涔?56    5涓?56--->128鐨勫嵎绉?            pts_channels=64,
            mid_channels=128,
            out_channels=128,
            ...
            activate_out=True,
            fuse_out=False),
        feature_fusion_layer=dict(
            type='feature_voxel_fusion',
            voxel_size=voxel_size,
            point_cloud_range=point_cloud_range,
            in_channels=140,
            out_channels=128,
        ...
        semantic_head=dict(
            type='ForegroundSegmentation',
            in_channels=128,
            extra_width=0.1,
            mlp_channels=(256, 256)),
        cro_attention_list=[
            dict(
                type='attentions_multi',
                dim=64,
                num_heads=4,
                layers=1
            ),
...
# resume = './work_dirs/pv_rcnn/epoch_7.pth'
# load_from = '<local-path>/涓嬭浇/code/lp/mmsegmentation-main/work_dirs/deeplabv3plus_r18-d8_4xb2-80k_cityscapes-512x1024/iter_40000.pth'
# load_from = '<local-path>/涓嬭浇/code/lp/UniMatch-main/result/trans.pth'
