...
from demo import convert_to_four_points, box_collision_test
...
def create_groundtruth_database(dataset_class_name,
    ...
    for j in track_iter_progress(list(range(len(dataset)))):
        ...
        occlusion_target = annos['occluded']
        # gt_corner_2d = convert_to_four_points(gt_boxes_2d)
        # coll_mask = box_collision_test(gt_corner_2d,gt_corner_2d)
        # diag = np.arange(gt_boxes_2d.shape[0])
        # coll_mask[diag, diag] = False

        path = data_info['lidar_path'].split('velodyne_reduced')
        img = mmcv.imread(path[0] + 'image_2' + path[1].split('bin')[0] + 'png')
        #
        ...
        for i, label in enumerate(data_info['gt_labels_3d']):
            if label == 2 or label == 3 or label == 4 or label == 5 or label == 6 or label == 7 or occlusion_target[i] !=0 :
                continue
            id = path[1].split('.bin')[0].split('/')[1]
            filename = f'{id}_{names[i]}_{i}.bin'
            abs_filepath = osp.join(database_save_path, filename)
            rel_filepath = osp.join(f'{info_prefix}_gt_database', filename)
            #  鏀瑰姩
            img_dir = database_save_path.split('kitti_gt_database')[0] + f'aug_img'
            if not os.path.exists(img_dir):
                os.mkdir(img_dir)
            _img_path = f'{id}_{names[i]}_{i}.png'
            image_path = os.path.join(img_dir, _img_path)
            crop_img = mmcv.imcrop(img, gt_boxes_2d[i])
            mmcv.imwrite(crop_img, image_path)
            # 鏀瑰姩
