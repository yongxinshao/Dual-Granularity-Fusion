...
from mmdet3d.tools.fps import DFPSSampler
import torch
...
def _create_train_reduced_point_cloud(data_path,
                                info_path,
                                save_path=None,
                                back=False,
                                num_features=4,
    ...
    for info in mmengine.track_iter_progress(kitti_infos):
        ...
        points_v = box_np_ops.remove_outside_points(points_v, rect, Trv2c, P2,
                                                    image_info['image_shape'])
        points_v = torch.from_numpy(points_v).to('cuda')
        fps_idx = fps(points_v.unsqueeze(0), 18000)
        points_v = points_v[fps_idx].squeeze(0).cpu().numpy()
        if save_path is None:
            save_dir = v_path.parent.parent / (v_path.parent.stem + '_reduced')
            if not save_dir.exists():
                save_dir.mkdir()
